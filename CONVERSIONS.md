...
**Chain:** `combine_files.py` then `jpg_pdf.py` then `llm_pdf_md.py`
...
`llm_pdf_md.py` then has to hand that whole thing to OpenAI's Vision API as one image.
...
`png_pdf.py`, combine those PDFs with `combine_files.py`, then run `llm_pdf_md.py`. You
